...
namespace oomph
{
  ...
  public:
    ...
    void initial_setup(Tree* const& adopted_father_pt = 0,
                       const unsigned& initial_p_order = 0) override;

    /// Pre-build (search father for required nodes which may already
    /// exist)
    void pre_build(Mesh*& mesh_pt, Vector<Node*>& new_node_pt) override;

    /// p-refine the element (refine if inc>0, unrefine if inc<0).
    void p_refine(const int& inc,
                  Mesh* const& mesh_pt,
                  GeneralisedElement* const& clone_pt) override;
    ...
    unsigned initial_p_order() const override
    {
  ...
  protected:
    ...
    void quad_hang_helper(const int& value_id,
                          const int& my_edge,
                          std::ofstream& output_hangfile) override;
  };
